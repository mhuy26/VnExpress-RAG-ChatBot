...
#         try:
#             vec = embedding.embed_query(doc.page_content[:100])
#             if len(vec) != expected_dim:
#                 print(f"❌ Chunk {i} has invalid vector size {len(vec)}")
#             else:
#                 valid_docs.append(doc)
#         except Exception as e:
#             print(f"⚠️ Error embedding chunk {i}: {e}")
#     print(f"✅ {len(valid_docs)} of {len(docs)} chunks passed vector check.")
#     return valid_docs
...
#     print(f"✂️ Total chunks before filtering: {len(splits)}")

#     # ✅ Check dimensions
#     splits = assert_vector_size(splits, embedding, vector_size)
#     if not splits:
#         print("❌ No valid documents to upload. Exiting.")
#         return
