...
def get_dynamic_k(user_query):
    ...
    """
    if "summary" or "tổng hợp" in user_query.lower():
        return 100
    else:
        return 5
...
def generate_response_stream(user_question: str, retrieved_content: str):
    """
    Stream chunks of response generated by the Gemini LLM using retrieved context.
    Yields:
        str: Streamed chunks of the generated answer.
    """
    # if retrieved_content.startswith(("⚠️", "🔌", "🔐", "📂", "🔍")):
    #     yield retrieved_content
    #     return
    ...
    try:
        ...
        # Use LangChain's stream generator
        ...
        print(f"❌ LLM streaming error in generate_response_stream: {type(e).__name__}: {str(e)}")
        yield f"⚠️ Failed to stream response: {str(e)}"
